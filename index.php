...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_request'])) {
    ...
    $exit_jdate  = trim($_POST['exit_date_jalali'] ?? '');
    $exit_time   = ($_POST['exit_time'] ?? '08:00') . ':00';
    $entry_jdate = trim($_POST['entry_date_jalali'] ?? '');
    $entry_time  = ($_POST['entry_time'] ?? '20:00') . ':00';
    ...
    if ($is_suspended) {
        ...
        if ($total_diff_hours > $soldier_info['leave_balance']) {
            ...
            if ($stmt->execute()) {
                ...
                if (!$request_id) {
                    die("خطا: آیدی درخواست خروج دریافت نشد!");
                }

                // حالا تأییدیه اولیه رو با تراکنش ایمن درج می‌کنیم
                try {
                    $stmt = $db->prepare("INSERT INTO approvals 
                (exit_request_id, user_id, approval_step, status, approved_at) 
                VALUES (?, ?, 1, 'approved', NOW())");

                    $stmt->execute([$request_id, $user_id]);   // این روش امن‌تر از bindParam هست

                    // اگر رسید اینجا یعنی با موفقیت درج شد
                    // ادامه منطق برنامه (چک کردن تأییدکننده بعدی و ...)

                } catch (PDOException $e) {
                    // دقیقاً بگو چه خطایی داده
                    die("خطا در درج تأییدیه: " . $e->getMessage());
                }
